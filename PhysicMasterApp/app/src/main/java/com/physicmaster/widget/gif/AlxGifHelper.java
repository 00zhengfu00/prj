package com.physicmaster.widget.gif;
...
public class AlxGifHelper {
    ...
    public static class ProgressViews {
        public ProgressViews(WeakReference<GifImageView> gifImageViewWeakReference,
                             WeakReference<ProgressWheel> progressWheelWeakReference,
                             WeakReference<TextView> textViewWeakReference, int displayWidth) {
    ...
    //防止同一个gif文件建立多个下载线程,url和imageView是一对多的关系,
    // 如果一个imageView建立了一次下载，那么其他请求这个url的imageView不需要重新开启一次新的下载，这几个imageView同时回调
     ...
     * @param gifView
     */
    public static void displayImage(final String url, GifImageView gifView, ProgressWheel
            progressBar, TextView tvProgress, int displayWidth) {
        //首先查询一下这个gif是否已被缓存
        String md5Url = getMd5(url);
        String path = gifView.getContext().getCacheDir().getAbsolutePath() + "/" + md5Url;//带
        // .tmp后缀的是没有下载完成的，用于加载第一帧，不带tmp后缀是下载完成的，
...
//            Log.i("AlexGIF", "本图片有缓存");
        ...
        final WeakReference<ProgressWheel> progressBarWait = new WeakReference<ProgressWheel>
                (progressBar);
        final WeakReference<TextView> textViewWait = new WeakReference<TextView>(tvProgress);
//        gifView.setImageResource(R.mipmap.pet_place_holder);//设置没有下载完成前的默认图片
...
//            Log.i("AlexGIF", "以前有别的ImageView申请加载过该gif" + url);
            ...
            memoryCache.get(url).add(new ProgressViews(imageViewWait, progressBarWait,
                    textViewWait, displayWidth));
            return;
        }
...
//                Log.i("AlexGIF", "下载gif的进度是" + progress + "%" + "    现在大小" + current + "   总大小" +
//                        total);
...
//                    if (tvProgress != null) tvProgress.setText(progress + "%");
                    //显示第一帧直到全部下载完之后开始动画
//                    getFirstPicOfGIF(new File(cacheFile.getAbsolutePath() + ".tmp"), vs
//                            .gifImageViewWeakReference.get());
                }
...
//                if (tvProgress != null) tvProgress.setText("image download failed");
     ...
     * @param gifImageView displayWidth
     *                     imageView控件的宽度，用于根据gif的实际高度重设控件的高度来保证完整显示，传0表示不缩放gif的大小，显示原始尺寸
     */
    public static boolean displayImage(File localFile, GifImageView gifImageView, int
            displayWidth) {
...
//            Log.i("AlexGIF", "图片原始height是" + raw_height + "  图片原始宽是:" + raw_width);
            if (gifImageView.getScaleType() != ImageView.ScaleType.CENTER_CROP && gifImageView
                    .getScaleType() != ImageView.ScaleType.FIT_XY) {
...
//                Log.i("AlexGIF", "按照固定大小进行显示");
     ...
     * @param task
     */
    public static void startDownLoad(final String uri, final File targetFile, final DownLoadTask
            task) {
     ...
     * @return
     */
    public static long downloadToStream(String uri, final File targetFile, final DownLoadTask
        ...
        try {
            try {
                ...
                if (HttpURLConnection.HTTP_OK == responseCode) {
                    ...
                    result = result < System.currentTimeMillis() ? System.currentTimeMillis() +
                            40000 : result;
                    fileLen = httpURLConnection.getContentLength();//这里通过http报文的header
                    // Content-Length来获取gif的总大小，需要服务器提前把header写好
                } else {
