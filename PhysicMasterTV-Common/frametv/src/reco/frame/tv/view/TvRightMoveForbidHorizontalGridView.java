 ...
 * @author keYence
 */
public class TvRightMoveForbidHorizontalGridView extends RelativeLayout {
    /**
     * 光标
     */
...
//                    scrollByRow(direction);
    ...
    public TvRightMoveForbidHorizontalGridView(Context context, AttributeSet attrs,
                                               int defStyle) {
        super(context, attrs, defStyle);
    }

    public TvRightMoveForbidHorizontalGridView(Context context) {
        super(context);
    }
    ...
    private void initItems() {
        ...
        //加header，独占两行
        if (header != null) {
            LayoutParams rlpHeader = new LayoutParams(itemWidth,
...
//        int initLength = adapter.getCount();
        ...
        colCount = ((itemIds.size() - headerCount) % rows == 0 ? (itemIds.size() - headerCount) /
                rows : (itemIds.size() - headerCount) / rows + 1) + headerCount;
    ...
    private void bindEventOnChild(View child, final int index) {
        ...
        child.setOnKeyListener(new OnKeyListener() {
            ...
            public boolean onKey(View v, int keyCode, KeyEvent event) {
                if (index / rows == colCount - headerCount - 1) {
...
//            if (savedNextCol != 0) {
            mScroller.startScroll(mScroller.getFinalX(), 0, -rowWidth, 0,
                    scrollDuration);
//            } else {
//                mScroller.startScroll(mScroller.getFinalX(), 0, -(int) getResources()
// .getDimension(R
//                        .dimen.px40), 0, scrollDuration);
//            }
        } else if (direction == View.FOCUS_RIGHT) {
            // mScroller.startScroll(0, mScroller.getFinalY(), 0, rowWidth,
            // scrollDuration);
//            if (savedNextCol != colCount - 1) {
            mScroller.startScroll(mScroller.getFinalX(), 0, rowWidth, 0,
                    scrollDuration);
//            } else {
//                mScroller.startScroll(mScroller.getFinalX(), 0, (int) getResources()
