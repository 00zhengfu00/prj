...
public class CommonCropImageView extends View {
    ...
    protected CropImageFloatDrawable mFloatDrawable;//浮层
    ...
    public CommonCropImageView(Context context) {
        super(context);
        init(context);
    }

    public CommonCropImageView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public CommonCropImageView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init(Context context) {
        ...
        mFloatDrawable = new CropImageFloatDrawable(context);//头像选择框
    }
...
//Log.v("count currentTouch"+currentTouch, "-------");	
    ...
    public int getTouchEdge(int eventX, int eventY) {
        ...
        if ((mFloatDrawable.getBounds().left - dx) <= eventX && eventX < (mFloatDrawable.getBounds().left + mFloatDrawable.getCirleWidth() + dx)
                && (mFloatDrawable.getBounds().top - dx) <= eventY && eventY < (mFloatDrawable.getBounds().top + mFloatDrawable.getCirleHeight() + dx)) {
            return EDGE_LT;//左上
        } else if ((mFloatDrawable.getBounds().right - mFloatDrawable.getCirleWidth() - dx) <= eventX && eventX < (mFloatDrawable.getBounds().right + dx)
                && (mFloatDrawable.getBounds().top - dx) <= eventY && eventY < (mFloatDrawable.getBounds().top + mFloatDrawable.getCirleHeight() + dx)) {
            return EDGE_RT;//右上
        } else if ((mFloatDrawable.getBounds().left - dx) <= eventX && eventX < (mFloatDrawable.getBounds().left + mFloatDrawable.getCirleWidth() + dx)
                && (mFloatDrawable.getBounds().bottom - mFloatDrawable.getCirleHeight() - dx) <= eventY && (eventY < mFloatDrawable.getBounds().bottom + dx)) {
            return EDGE_LB;//左下
        } else if ((mFloatDrawable.getBounds().right - mFloatDrawable.getCirleWidth() - dx) <= eventX && eventX < (mFloatDrawable.getBounds().right + dx)
                && (mFloatDrawable.getBounds().bottom - mFloatDrawable.getCirleHeight() - dx) <= eventY && eventY < (mFloatDrawable.getBounds().bottom + dx)) {
            return EDGE_RB;//右下
...
//		super.onDraw(canvas);
    ...
    private int getViewVisibleWidth() {
        return getWidth() - 2 * mFloatDrawable.getCirleWidth();
    }
//	private int getViewVisibleHeight(){
//		return getHeight() - 2*mFloatDrawable.getCirleHeight();
//	}
    ...
    protected void configureBounds() {
        if (isFrist) {
            ...
            int w = Math.min(getViewVisibleWidth(), (int) (mDrawable.getIntrinsicWidth() * scale + 0.5f));
...
//			
            ...
            int floatWidth = w;
            int floatHeight = Math.min(h, getHeight() - 2 * mFloatDrawable.getCirleWidth());
    ...
    public Bitmap getCropImage() {
        ...
        if ((width <= 0) || (height <= 0)) {
            Log.e("CommonCropImageView", "getCropImage failed!" + "-------mDrawableFloat.left:" + mDrawableFloat.left + "top:" + mDrawableFloat.top + "right:" + mDrawableFloat
                    .right + "bottom:" + mDrawableFloat.bottom);
            return null;
        }

        if (width + mDrawableFloat.left > tmpBitmap.getWidth()) {
            width = tmpBitmap.getWidth() - mDrawableFloat.left;
        }
        if (height + mDrawableFloat.top > tmpBitmap.getHeight()) {
            height = tmpBitmap.getHeight() - mDrawableFloat.top;
        }

        Bitmap ret = Bitmap.createBitmap(tmpBitmap, mDrawableFloat.left, mDrawableFloat.top, width, height, matrix, true);
        tmpBitmap.recycle();
        tmpBitmap = null;
