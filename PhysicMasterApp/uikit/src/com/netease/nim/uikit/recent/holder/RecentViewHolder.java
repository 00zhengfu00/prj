...
public abstract class RecentViewHolder extends RecyclerViewHolder<BaseQuickAdapter,
        BaseViewHolder, RecentContact> {
    ...
    @Override
    public void convert(BaseViewHolder holder, RecentContact data, int position, boolean
            isScrolling) {
        inflate(holder, data);
    ...
    public void refresh(BaseViewHolder holder, RecentContact recent, final int position) {
        ...
        String nickName = UserInfoHelper.getUserTitleName(recent.getContactId(), recent
                .getSessionType());
        if (nickName.equals(recent.getContactId())) {
            updateNickLabel(recent.getFromNick());
        } else {
            updateNickLabel(nickName);
        }
            ...
            if (o instanceof String && o.equals("0")) {
                ...
                new Handler().post(new Runnable() {
                    ...
                    public void run() {
                        ...
                        getAdapter().notifyItemChanged(getAdapter().getViewHolderPosition
                                (position));
                    }
                });
            }
    ...
    private void updateBackground(BaseViewHolder holder, RecentContact recent, int position) {
        ...
        if ((recent.getTag() & RecentContactsFragment.RECENT_TAG_STICKY) == 0) {
            ...
            holder.getConvertView().setBackgroundResource(R.drawable
                    .nim_recent_contact_sticky_selecter);
        }
    }
    ...
    private void updateMsgLabel(BaseViewHolder holder, RecentContact recent) {
        // 显示消息具体内容
        MoonUtil.identifyRecentVHFaceExpressionAndTags(holder.getContext(), tvMessage, getContent
                (recent), -1, 0.45f);
