...
public class LoginActivity extends BaseActivity implements View.OnClickListener {
    ...
    protected void initView() {
        ...
        if (intent != null) {
            if (intent.getBooleanExtra(KICK_OUT, false)) {
                ...
                EasyAlertDialogHelper.showOneButtonDiolag(LoginActivity.this, getString(R.string
                                .kickout_notify),
        ...
        //清除首页残留数据(如果首页还未销毁的话)
        LocalBroadcastManager.getInstance(LoginActivity.this).sendBroadcast(new Intent(StudyFragmentV2.ON_SUBJECT_CHANGED));
    }
    ...
    private void login2QQNOWL() {
        ...
        if (null != dataBean) {
            ...
            UIUtils.showToast(LoginActivity.this, "数据异常");
            finish();
        }
    }
    ...
    private void doLoigin() {
        ...
        if (TextUtils.isEmpty(phoneNum) || TextUtils.isEmpty(password)) {
            UIUtils.showToast(LoginActivity.this, "请输入用户名或密码");
            return;
        }
    ...
    private void loginAfterAction(UserDataResponse.UserDataBean data) {
        ...
        if (data.loginVo.isInitial == 0) {
            Intent intent = new Intent(LoginActivity.this, SelectPetActivity.class);
            UIUtils.showToast(LoginActivity.this, "请完善基本信息");
            startActivity(intent);
        } else {
