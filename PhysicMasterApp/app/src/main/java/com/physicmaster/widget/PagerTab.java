...
public class PagerTab extends ViewGroup {
    ...
    private int mDividerPadding = 7;// 分割线上下的padding
    ...
    private int mIndicatorPadding ;//指示器的距离左边的距离
    private int mIndicatorColor = 0xFF07A7FA;//指示器颜色
    ...
    private int mTabBackgroundResId = R.drawable.information_title_selector;// tab背景资源
    private int mTabTextColorResId = R.drawable.information_text_selector; //tab文字颜色
    ...
    public PagerTab(Context context, AttributeSet attrs, int defStyle) {
        ...
        mDividerWidth = typedArray.getDimensionPixelSize(R.styleable.PagerTab_dividerWidth,
                mDividerWidth);
        mIndicatorHeight = typedArray.getDimensionPixelSize(R.styleable.PagerTab_indicatorHeight,
                mIndicatorHeight);
        mIndicatorWidth = typedArray.getDimensionPixelSize(R.styleable.PagerTab_indicatorWidth,
                mIndicatorWidth);
        mIndicatorPadding = typedArray.getDimensionPixelSize(R.styleable.PagerTab_mIndicatorPadding,
                mIndicatorPadding);
        mDividerColor = typedArray.getColor(R.styleable.PagerTab_dividerColor, mDividerColor);
        mTabBackgroundResId = typedArray.getResourceId(R.styleable.PagerTab_tabBackground,
                mTabBackgroundResId);
        mTabTextColorResId = typedArray.getResourceId(R.styleable.PagerTab_tabTxtColor,
                mTabTextColorResId);
        mTabTextSize = typedArray.getDimensionPixelSize(R.styleable.PagerTab_tabTextSize,
                mTabTextSize);
        typedArray.recycle();
        init();
        initPaint();
    ...
    private void init() {
        //把一个值从dip转换成px
        mIndicatorHeight = ScreenUtils.dp2px(mContext, mIndicatorHeight);
        mDividerPadding = ScreenUtils.dp2px(mContext, mDividerPadding);
        mTabPadding = ScreenUtils.dp2px(mContext, mTabPadding);
        mIndicatorPadding = ScreenUtils.dp2px(mContext, mIndicatorPadding);
        //创建一个scroller
    ...
    private void addTextTab(final int position, String title) {
        RelativeLayout view = (RelativeLayout) LayoutInflater.from(mContext).inflate(R.layout.pagertab_title, this, false);
        TextView tab = (TextView) view.findViewById(R.id.tv_title);
        TextView msgNum = (TextView) view.findViewById(R.id.tv_msg_num);
        tvMsgs.add(msgNum);
//        TextView tab = new TextView(mContext);
        ...
        tab.setTextColor(ScreenUtils.getColorStateList(mTabTextColorResId));
        view.setBackgroundDrawable(ScreenUtils.getDrawable(mTabBackgroundResId));
...
//        if (childCount>=3) {
//            mIndicatorPadding = ScreenUtils.dp2px(mContext, 10);
//        } else{
//            mIndicatorPadding = ScreenUtils.dp2px(mContext, 50);
//        }

        //画指示器
        canvas.drawRect(mIndicatorLeft + mIndicatorPadding, height - mIndicatorHeight, mIndicatorLeft + mIndicatorWidth - mIndicatorPadding, height, mIndicatorPaint);
    ...
    private void checkAndcalculate() {
        ...
        if (mIndicatorLeft < firstTab.getLeft()) {
            ...
            mIndicatorWidth = firstTab.getWidth() - 10;
        }
        ...
        for (int i = 0; i < mTabCount; i++) {
            ...
            if (mIndicatorLeft < tab.getLeft()) {
                ...
                mCurrentOffsetPixels = (mIndicatorLeft - currentTab.getLeft()) / (currentTab
                        .getWidth() + 0.0f);
                break;
            }
        }
    ...
    public void scrollSelf(int position, float offset) {
        ...
        if (offset > 0 && rightPosition < mTabCount) {
            ...
            mIndicatorWidth = (int) (tab.getWidth() * (1 - offset) + rightTab.getWidth() * offset
                    + 0.5);
        } else {
    ...
    private class PageListener implements OnPageChangeListener {
        @Override
        public void onPageScrolled(int position, float positionOffset, final int
    ...
    public void setMsgs(int msgs[]) {
        this.msgs = msgs;
        if (msgs[0] == 0) {
            tvMsgs.get(0).setVisibility(GONE);
        } else {
            tvMsgs.get(0).setVisibility(VISIBLE);
            tvMsgs.get(0).setText(msgs[0] + "");
        }
        if (msgs[1] == 0) {
            tvMsgs.get(1).setVisibility(GONE);
        } else {
            tvMsgs.get(1).setVisibility(VISIBLE);
            tvMsgs.get(1).setText(msgs[1] + "");
        }
        if (msgs[2] == 0) {
            tvMsgs.get(2).setVisibility(GONE);
        } else {
            tvMsgs.get(2).setVisibility(VISIBLE);
            tvMsgs.get(2).setText(msgs[2] + "");
        }
    }
}
