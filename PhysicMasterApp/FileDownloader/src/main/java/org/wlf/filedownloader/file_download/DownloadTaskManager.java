...
public class DownloadTaskManager implements Pauseable {
     ...
     */
    private Map<String, DownloadTask> mRunningDownloadTaskMap = new ConcurrentHashMap<String,
            DownloadTask>();
     ...
     */
    public DownloadTaskManager(FileDownloadConfiguration configuration, DownloadRecorder
            downloadRecorder) {
    ...
    private DownloadTask getRunningDownloadTask(String url, boolean includeStopped) {
        ...
        // not in recorder, maybe the DownloadFileInfo not create(download status before
        // prepared), look up the
     ...
     */
    public void registerDownloadStatusListener(OnFileDownloadStatusListener
     ...
     * @since 0.2.0
     */
    public void unregisterDownloadStatusListener(OnFileDownloadStatusListener
                                                         onFileDownloadStatusListener) {
     ...
     */
    private void addAndRunDetectUrlFileTask(String url, String videoId, boolean forceDetect,
        ...
        // ------------start checking conditions & notifying caller if necessary------------
        {
            ...
            if (!UrlUtil.isUrl(url)) {
                failReason = new DetectUrlFileFailReason(url, "url illegal !",
            ...
            if (failReason == null && !NetworkUtil.isNetworkAvailable(mConfiguration.getContext()
                ...
                failReason = new DetectUrlFileFailReason(url, "network not available !",
                        DetectUrlFileFailReason
                                .TYPE_NETWORK_DENIED);
        ...
        DetectUrlFileTask detectUrlFileTask = new DetectUrlFileTask(url, mConfiguration
                .getFileDownloadDir(), videoId,
    ...
    private void addAndRunDownloadTask(final String callerUrl, DownloadFileInfo downloadFileInfo,
        ...
        if (failReason == null) {
            if (!DownloadFileUtil.isLegal(downloadFileInfo)) {
                failReason = new OnFileDownloadStatusFailReason(callerUrl, "the download file " +
                        "does not exist or " +
                        "illegal !", OnFileDownloadStatusFailReason.TYPE_DOWNLOAD_FILE_ERROR);
            }
            if (failReason == null) {
                if (TextUtils.isEmpty(callerUrl) || !callerUrl.equals(downloadFileInfo.getUrl())
                        || !downloadFileInfo
        ...
        if (failReason == null && downloadFileInfo.getDownloadedSizeLong() > downloadFileInfo
                .getFileSizeLong()) {
            failReason = new OnFileDownloadStatusFailReason(callerUrl, "download size illegal, " +
                    "please delete or " +
        ...
        synchronized (mDownloadTaskLock) {
            ...
            if (taskInMap != null) {
                ...
                Log.d(TAG, "mRunningDownloadTaskMap，忽略1：" + callerUrl + "，old task：" + taskInMap
                        .hashCode() +
                        "，线程数：" + threads.size());
                return;
            }
        ...
        final RetryableDownloadTaskImpl downloadTask = new RetryableDownloadTaskImpl
                (FileDownloadTaskParam
                        .createByDownloadFile(downloadFileInfo, requestMethod, headers),
                        mDownloadRecorder,
        ...
        downloadTask.setOnTaskRunFinishListener(new OnTaskRunFinishListener() {
            ...
            public void onTaskRunFinish() {
                ...
                synchronized (mDownloadTaskLock) {
                    ...
                    Map<Thread, StackTraceElement[]> threads = Thread.getAllStackTraces();
                    Log.e(TAG, "mRunningDownloadTaskMap，--移除--：" + downloadTask.getUrl() +
                            "，task：" + downloadTask
        ...
        synchronized (mDownloadTaskLock) {
            ...
            if (taskInMap != null) {
                // running, ignore
                Map<Thread, StackTraceElement[]> threads = Thread.getAllStackTraces();
                Log.d(TAG, "mRunningDownloadTaskMap，忽略2：" + downloadTask.getUrl() + "，old task："
                        + taskInMap.hashCode
                        () + "，线程数：" + threads.size());
                return;
            }
            ...
            Map<Thread, StackTraceElement[]> threads = Thread.getAllStackTraces();
            Log.d(TAG, "mRunningDownloadTaskMap，--增加--：" + downloadTask.getUrl() + "，task：" +
                    downloadTask.hashCode()
                    + "，线程数：" + threads.size());
        }
        // exec the task
    ...
    private void notifyDetectUrlFileFailed(String url, DetectBigUrlFileFailReason failReason,
        ...
        OnDetectBigUrlFileListener.MainThreadHelper.onDetectUrlFileFailed(url, failReason,
                onDetectBigUrlFileListener);
    }
     ...
     */
    private boolean notifyDownloadStatusFailed(String url, final FileDownloadStatusFailReason
            failReason, final
        ...
        if (isDownloading(url)) {
            ...
            pause(url, new OnStopFileDownloadTaskListener() {
                ...
                @Override
                public void onStopFileDownloadTaskFailed(String url,
                                                         StopDownloadFileTaskFailReason
                                                                 stopFailReason) {
                    // continue notify caller
     ...
     */
    private boolean notifyDownloadStatusFailedInternal(String url, FileDownloadStatusFailReason
            failReason, boolean
                                                               recordStatus) {
        if (recordStatus) {
            // record status
            try {
                ...
                // notify observer
                mDownloadStatusObserver.onFileDownloadStatusFailed(url, getDownloadFile(url),
                        failReason);
                return true;
            } catch (Exception e) {
                e.printStackTrace();
     ...
     */
    private void detectInternal(String url, String videoId, boolean forceDetect,
                                OnDetectBigUrlFileListener
                                        onDetectBigUrlFileListener, DownloadConfiguration
                                        downloadConfiguration) {
        // start detect task
        addAndRunDetectUrlFileTask(url, videoId, forceDetect, onDetectBigUrlFileListener,
                downloadConfiguration);
    }
     ...
     * @deprecated this method can not detect the url file which bigger than 2G, use {@link
     * #detect(String, int
     * OnDetectBigUrlFileListener, DownloadConfiguration)}instead
     */
    @Deprecated
    public void detect(String url, String videoId, final OnDetectUrlFileListener
            onDetectUrlFileListener,
                       DownloadConfiguration
                               downloadConfiguration) {
        final String finalUrl = url;
        detect(finalUrl, videoId, new OnDetectBigUrlFileListener() {// inner change callback
            @Override
            public void onDetectNewDownloadFile(String url, String fileName, String saveDir, long
                    fileSize) {
                // continue notify caller
                if (onDetectUrlFileListener != null) {
                    onDetectUrlFileListener.onDetectNewDownloadFile(finalUrl, fileName, saveDir,
                            (int) fileSize);
                }
            }
            ...
            public void onDetectUrlFileFailed(String url, DetectBigUrlFileFailReason failReason) {
                // continue notify caller
                if (onDetectUrlFileListener != null) {
                    onDetectUrlFileListener.onDetectUrlFileFailed(finalUrl, new
                            DetectUrlFileFailReason(finalUrl,
                            failReason));
                }
            }
        }, downloadConfiguration);
     ...
     * @param downloadConfiguration      download configuration
     */
    public void detect(String url, String videoId, OnDetectBigUrlFileListener
            onDetectBigUrlFileListener,
                       DownloadConfiguration
                               downloadConfiguration) {
        detectInternal(url, videoId, false, onDetectBigUrlFileListener, downloadConfiguration);
    }
    ...
    /**
     * create a new download after detected a url file by using {@link #detect(String, int
     * OnDetectBigUrlFileListener,
     * DownloadConfiguration)}
     * <br/>
     ...
     */
    private void createAndStartByDetectUrlFile(String callerUrl, DetectUrlFileInfo
            detectUrlFileInfo,
                                               DownloadConfiguration downloadConfiguration) {
        ...
        if (!DownloadFileUtil.isLegal(detectUrlFileInfo)) {
            failReason = new OnFileDownloadStatusFailReason(callerUrl, "detect file does not " +
                    "exist, please use detect" +
                    "(String," + "OnDetectBigUrlFileListener) first !",
                    OnFileDownloadStatusFailReason
                            .TYPE_FILE_NOT_DETECT);
            if (failReason == null) {
                if (!UrlUtil.isUrl(callerUrl) || !callerUrl.equals(detectUrlFileInfo.getUrl())) {
                    failReason = new OnFileDownloadStatusFailReason(callerUrl, "detect file does " +
                            "not exist, please " +
        ...
        DownloadFileInfo downloadFileInfo = mDownloadRecorder.createDownloadFileInfo
                (detectUrlFileInfo);
     ...
     * @param downloadConfiguration download configuration
     */
    public void start(String url, String videoId, final DownloadConfiguration
            downloadConfiguration) {
        DownloadFileInfo downloadFileInfo = getDownloadFile(url);
        // has been downloaded
        ...
        // not download
            ...
            // not detect
                ...
                final String finalUrl = url;
                // detect first
                detect(finalUrl, videoId, new OnDetectBigUrlFileListener() {
                    @Override
                    public void onDetectUrlFileFailed(String url, DetectBigUrlFileFailReason
                            failReason) {
                        // notify download status caller
                        notifyDownloadStatusFailed(finalUrl, new OnFileDownloadStatusFailReason
                                        (finalUrl, failReason)
                                , false);
                    }
                    ...
                    @Override
                    public void onDetectNewDownloadFile(String url, String fileName, String
                            savedDir, long fileSize) {
     ...
     * @param downloadConfiguration download configuration
     */
    public void start(List<String> urls, String videoId, DownloadConfiguration
            downloadConfiguration) {
        for (String url : urls) {
            start(url, videoId, downloadConfiguration);
        }
    }
     ...
     */
    private void pauseInternal(String url, final OnStopFileDownloadTaskListener
            onStopFileDownloadTaskListener) {

        // get download task info
        DownloadTask downloadTask = getRunningDownloadTask(url);

        Log.d(TAG, "pauseInternal fileDownloadTask是否已经停止：" + (downloadTask == null || downloadTask
                .isStopped()) +
                ",url：" + url);
        ...
        if (downloadTask != null && !downloadTask.isStopped()) {
            ...
            downloadTask.setOnStopFileDownloadTaskListener(new OnStopFileDownloadTaskListener() {
                ...
                public void onStopFileDownloadTaskFailed(String url,
                    ...
                    Log.d(TAG, "pauseInternal 暂停失败，url：" + finalUrl + ",failReason:" + failReason
                            .getType());

                    // notify caller
                    notifyStopDownloadTaskFailed(finalUrl, failReason,
                            onStopFileDownloadTaskListener);
                }
            });
            ...
            StopDownloadFileTaskFailReason failReason = new StopDownloadFileTaskFailReason
                    (finalUrl, "the download "
                            + "task has been paused !", StopDownloadFileTaskFailReason
                            .TYPE_TASK_HAS_BEEN_STOPPED);
     ...
     */
    public void pause(List<String> urls, OnStopFileDownloadTaskListener
            onStopFileDownloadTaskListener) {
        for (String url : urls) {
    ...
    // --------------------------------------restart downloads--------------------------------------
    public void restartAll(OnStopFileDownloadTaskListener onStopFileDownloadTaskListener) {
        Set<String> urls = mRunningDownloadTaskMap.keySet();
        reStart(new ArrayList<String>(urls), "", null);
    }

    /**
     * restart a download
     */
    private void reStartInternal(String url, final String videoId, final boolean isDelete, final
    DownloadConfiguration
            downloadConfiguration) {

        // 1.check url
        if (!UrlUtil.isUrl(url)) {
            FileDownloadStatusFailReason failReason = new OnFileDownloadStatusFailReason(url,
                    "url illegal !",
                    OnFileDownloadStatusFailReason.TYPE_URL_ILLEGAL);
            // notify caller
        ...
        if (DownloadFileUtil.isLegal(downloadFileInfo)) {
            ...
            // 2.detect
            detectInternal(url, videoId, true, new OnDetectBigUrlFileListener() {
                @Override
                public void onDetectNewDownloadFile(String url, String fileName, String saveDir,
                    ...
                    try {
                        ...
                        FileDownloadStatusFailReason failReason = new
                                OnFileDownloadStatusFailReason(finalUrl, e);
                        // notify caller
                        notifyDownloadStatusFailed(finalUrl, failReason, getDownloadFile
                                (finalUrl) != null);
                    }
                }
     ...
     * @param downloadConfiguration download configuration
     */
    public void reStart(String url, final String videoId, final DownloadConfiguration
            downloadConfiguration) {
        ...
        if (isDownloading(url)) {
            ...
            pause(url, new OnStopFileDownloadTaskListener() {
                ...
                public void onStopFileDownloadTaskSucceed(String url) {
                    ...
                    reStartInternal(finalUrl, videoId, true, downloadConfiguration);
                }
                ...
                public void onStopFileDownloadTaskFailed(String url,
                    ...
                    notifyDownloadStatusFailed(finalUrl, failReason, getDownloadFile(finalUrl) !=
                            null);
                }
            });
        } else {
            // has been stopped, restart directly
            reStartInternal(url, videoId, true, downloadConfiguration);
        }
    }
     ...
     * @param downloadConfiguration download configuration
     */
    public void reStart(List<String> urls, String videoId, DownloadConfiguration
            downloadConfiguration) {
        for (String url : urls) {
            reStart(url, videoId, downloadConfiguration);
        }
    }
