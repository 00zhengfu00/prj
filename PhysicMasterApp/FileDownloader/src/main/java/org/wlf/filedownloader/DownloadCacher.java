...
//                mDownloadFileInfoMap.put(downloadFileInfo.getUrl(), downloadFileInfo);
    ...
    public List<DownloadFileInfo> getDownloadInfoByParentId(String parentId) {
        // read from database
        ContentDbDao dao = mDownloadFileDbHelper.getContentDbDao(DownloadFileInfo.Table
                .TABLE_NAME_OF_DOWNLOAD_FILE);
        if (dao == null) {
        ...
        // query all
        Cursor cursor = dao.query(null, Table.COLUMN_NAME_OF_FIELD_PARENT_ID + "= ?", new
...
//                mDownloadFileInfoMap.put(url, downloadFileInfo);
     ...
     */
    private boolean updateDownloadFileInternal(DownloadFileInfo downloadFileInfo, boolean
            lockInternal, Type
                                                       notifyType) {
...
//                    mDownloadFileInfoMap.remove(url);
     ...
     * @return DownloadFile
     */
    public DownloadFileInfo getDownloadFileBySavePath(String savePath, boolean
            includeTempFilePath) {
...
//        }
        ...
        if (downloadFileInfo == null) {
            ...
            Cursor cursor = dao.query(null, DownloadFileInfo.Table.COLUMN_NAME_OF_FIELD_FILE_DIR
                    + "= ? AND " +
                    DownloadFileInfo.Table.COLUMN_NAME_OF_FIELD_FILE_NAME + "= ?", new
                    String[]{fileSaveDir,
                    fileSaveName}, null);
            ...
            if (downloadFileInfo == null) {
                if (includeTempFilePath) {
                    ...
                    cursor = dao.query(null, DownloadFileInfo.Table.COLUMN_NAME_OF_FIELD_FILE_DIR
                            + "= ? AND " +
                            Table.COLUMN_NAME_OF_FIELD_TEMP_FILE_NAME + "= ?", new
                            String[]{fileSaveDir,
                            fileSaveName}, null);
                    if (cursor != null && cursor.moveToFirst()) {
                        downloadFileInfo = new DownloadFileInfo(cursor);
...
//        }
    ...
    public void deleteDownloadFile(String url) throws Exception {
        ...
        if (!isSucceed) {
            // really deleted ?
            DownloadFileInfo DownloadFileInfoDeleted = getDownloadFileInternal(downloadFileInfo
                    .getUrl());
    ...
    public boolean deleteDownloadFileByParentId(String parentId) {
        ...
        //1.删除指定parentId的ts文件
        List<DownloadFileInfo> fileInfoList = getDownloadInfoByParentId(parentId);
        if (fileInfoList != null) {
            for (DownloadFileInfo downloadFileInfo : fileInfoList) {
        ...
        //2.删除数据库记录
        synchronized (mModifyLock) {// lock
            int result = dao.delete(Table.COLUMN_NAME_OF_FIELD_PARENT_ID + "= ?", new
                    String[]{parentId});
            if (result >= 1) {
                return true;
            }
            List<DownloadFileInfo> fileInfoListExist = getDownloadInfoByParentId(parentId);
