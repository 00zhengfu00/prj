package com.physicmaster.modules.discuss.session;
...
import com.physicmaster.modules.discuss.contact.activity.UserProfileActivity;
import com.physicmaster.modules.discuss.session.action.SnapChatAction;
import com.physicmaster.modules.discuss.session.action.TipAction;
import com.physicmaster.modules.discuss.session.activity.MessageHistoryActivity;
import com.physicmaster.modules.discuss.session.activity.MessageInfoActivity;
import com.physicmaster.modules.discuss.session.extension.CustomAttachParser;
import com.physicmaster.modules.discuss.session.extension.CustomAttachment;
import com.physicmaster.modules.discuss.session.extension.GuessAttachment;
import com.physicmaster.modules.discuss.session.extension.RTSAttachment;
...
import com.physicmaster.modules.discuss.session.search.SearchMessageActivity;
import com.physicmaster.modules.discuss.session.viewholder.MsgViewHolderDefCustom;
import com.physicmaster.modules.discuss.session.viewholder.MsgViewHolderFile;
import com.physicmaster.modules.discuss.session.viewholder.MsgViewHolderGuess;
import com.physicmaster.modules.discuss.session.viewholder.MsgViewHolderSticker;
import com.physicmaster.modules.discuss.session.viewholder.MsgViewHolderTip;
...
public class SessionHelper {
    ...
    public static void startTeamSession(Context context, String tid, Class<? extends Activity>
            backToClass, IMMessage anchor) {
    ...
    private static SessionCustomization getP2pCustomization() {
        if (p2pCustomization == null) {
            p2pCustomization = new SessionCustomization() {
                ...
                @Override
                public void onActivityResult(final Activity activity, int requestCode, int
...
//            actions.add(new FileAction());
            ...
            SessionCustomization.OptionsButton cloudMsgButton = new SessionCustomization
                    .OptionsButton() {
                @Override
            ...
            SessionCustomization.OptionsButton infoButton = new SessionCustomization
                    .OptionsButton() {
                @Override
                public void onClick(Context context, View view, String sessionId) {
        ...
        if (myP2pCustomization == null) {
            myP2pCustomization = new SessionCustomization() {
                ...
                public void onActivityResult(Activity activity, int requestCode, int resultCode,
                                             Intent data) {
                    if (requestCode == TeamRequestCode.REQUEST_CODE && resultCode == Activity
                            .RESULT_OK) {
            ...
            SessionCustomization.OptionsButton infoButton = new SessionCustomization
                ...
                public void onClick(Context context, View view, String sessionId) {
                    ...
                    if (team != null && team.isMyTeam()) {
                        ...
                        Toast.makeText(context, R.string.team_invalid_tip, Toast.LENGTH_SHORT)
                                .show();
                    }
                }
            };
        ...
        NimUIKit.setMsgForwardFilter(new MsgForwardFilter() {
            ...
            public boolean shouldIgnore(IMMessage message) {
                if (message.getDirect() == MsgDirectionEnum.In
                    ...
                    return true;
                } else if (message.getMsgType() == MsgTypeEnum.custom && message.getAttachment()
                        != null
...
//                }
    ...
    private static NIMPopupMenu.MenuItemClickListener listener = new NIMPopupMenu
            .MenuItemClickListener() {
        @Override
        public void onItemClick(final PopupMenuItem item) {
            switch (item.getTag()) {
                case ACTION_HISTORY_QUERY:
                    MessageHistoryActivity.start(item.getContext(), item.getSessionId(), item
                            .getSessionTypeEnum()); // 漫游消息查询
                    break;
                case ACTION_SEARCH_MESSAGE:
                    SearchMessageActivity.start(item.getContext(), item.getSessionId(), item
                            .getSessionTypeEnum());
                    break;
                case ACTION_CLEAR_MESSAGE:
                    EasyAlertDialogHelper.createOkCancelDiolag(item.getContext(), null,
                            "确定要清空吗？", true, new EasyAlertDialogHelper.OnDialogActionListener() {
                                @Override
                                ...
                                public void doOkAction() {
                                    NIMClient.getService(MsgService.class).clearChattingHistory(item
                                            .getSessionId(), item.getSessionTypeEnum());
                                    MessageListPanelHelper.getInstance().notifyClearMessages(item
                                            .getSessionId());
                                }
                            }).show();
                    break;
