...
public class RewardFragment extends BaseFragment {
    ...
    private void initListView() {
        ...
        obtainedItems = new ArrayList<ObtainedItem>();
        adapter = new RewardAdapter(obtainedItems, getActivity());
    ...
    private void getData(int itemId) {
        ObtainedItemService service = new ObtainedItemService(getActivity());
        service.setCallback(new IOpenApiDataServiceCallback<ObtainedItemResponse>() {
            @Override
            public void onGetData(ObtainedItemResponse data) {
                try {
                    if (id == 0) {
                        obtainedItems.clear();
                        setFooterStatus(false);
                    }
                    if (data.data.obtainedItem.size() > 0) {
                        obtainedItems.addAll(data.data.obtainedItem);
                        id = obtainedItems.get(obtainedItems.size() - 1).getItemId();
                    } else {
