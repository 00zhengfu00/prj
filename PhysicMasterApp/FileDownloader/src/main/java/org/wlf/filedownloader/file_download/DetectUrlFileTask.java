...
class DetectUrlFileTask implements Runnable {
    ...
    public DetectUrlFileTask(String url, String downloadSaveDir, String videoId, DetectUrlFileCacher
            detectUrlFileCacher, DownloadRecorder downloadRecorder) {
        super();
        this.mUrl = url;
     ...
     */
    public void setOnDetectBigUrlFileListener(OnDetectBigUrlFileListener
    ...
    public void run() {
         ...
         * 2.if url file detected and not force detect flag, ignore, make sure do not detect
         * duplicate
         ...
         *
         * note: DetectUrlFileFailReason extends from DetectBigUrlFileFailReason, so they are
         * compatible
         */
        ...
        try {
            // ------------start checking conditions------------
            {
                ...
                if (!UrlUtil.isUrl(mUrl)) {
                    // error, url illegal
                    failReason = new DetectUrlFileFailReason(mUrl, "url illegal !",
            ...
            conn = HttpConnectionHelper.createDetectConnection(connectUrl, mConnectTimeout,
                    mCharset, mRequestMethod,
                    mHeaders);
            ...
            while (conn.getResponseCode() / 100 == 3 && redirectCount < MAX_REDIRECT_TIMES) {
                ...
                conn = HttpConnectionHelper.createDetectConnection(connectUrl, mConnectTimeout,
                        mCharset, mRequestMethod, mHeaders);
                redirectCount++;
            }

            Log.d(TAG, TAG + ".run 探测文件，重定向：" + redirectCount + "次" + "，最大重定向次数：" +
                    MAX_REDIRECT_TIMES +
                    "，url：" + mUrl);

            if (redirectCount > MAX_REDIRECT_TIMES) {
                // error over max redirect
                failReason = new DetectUrlFileFailReason(mUrl, "over max redirect:" +
                        MAX_REDIRECT_TIMES + "!",
            ...
            Log.i(TAG, TAG + ".run Response Headers:" + HttpConnectionHelper.getStringHeaders
            ...
            switch (conn.getResponseCode()) {
                ...
                case HttpURLConnection.HTTP_OK:
                    ...
                    // get file name
                    String fileName = HttpConnectionHelper.getFileNameFromResponseHeader(conn
                            .getHeaderFields());
                    ...
                    String lastModified = HttpConnectionHelper.getLastModifiedFromResponseHeader
                            (conn.getHeaderFields
                            ());
                    ...
                    if (fileSize <= 0) {
                        fileSize = HttpConnectionHelper.getFileSizeFromResponseHeader(conn
                                .getHeaderFields());
                    }

                    if (fileSize > 0) {
                        detectUrlFileInfo = new DetectUrlFileInfo(mUrl, fileSize, eTag,
                                lastModified,
                                acceptRangeType, mDownloadSaveDir, fileName, mVideoId+"");
                ...
                default:
                    ...
                    failReason = new DetectUrlFileFailReason(mUrl, "ResponseCode:" + conn
                            .getResponseCode() + " " +
                            "error, can not read data !", DetectUrlFileFailReason
                            .TYPE_BAD_HTTP_RESPONSE_CODE);
                    break;
            }
        } catch (Exception e) {
            e.printStackTrace();
            ...
            // ------------start notifying caller------------
            {
                ...
                if (!isNotify && detectUrlFileInfo != null) {
                    ...
                    try {
                        ...
                        isNotify = notifyDetectNewDownloadFile(detectUrlFileInfo.getFileName(),
                                detectUrlFileInfo
                ...
                if (!isNotify) {
                    if (failReason == null) {
                        failReason = new DetectUrlFileFailReason(mUrl, "the file need to download" +
                                " may not access !",
            ...
            boolean hasException = failReason == null;

            Log.d(TAG, TAG + ".run 探测文件任务【已结束】，是否有异常：" + hasException + "，是否强制探测模式：" +
                    mIsForceDetect +
    ...
    private boolean notifyDetectUrlFileExist() {
        ...
        if (mOnDetectBigUrlFileListener != null) {
            ...
            OnDetectBigUrlFileListener.MainThreadHelper.onDetectUrlFileExist(mUrl,
    ...
    private boolean notifyDetectNewDownloadFile(String fileName, String saveDir, long fileSize) {
        ...
        if (mOnDetectBigUrlFileListener != null) {
            // main thread notify caller
            OnDetectBigUrlFileListener.MainThreadHelper.onDetectNewDownloadFile(mUrl, fileName,
                    saveDir, fileSize,
                    mOnDetectBigUrlFileListener);
            // return true;
        }
