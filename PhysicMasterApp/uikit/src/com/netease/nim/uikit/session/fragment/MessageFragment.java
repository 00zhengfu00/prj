...
public class MessageFragment extends TFragment implements ModuleProxy {
    ...
    public void onPause() {
        super.onPause();
        NIMClient.getService(MsgService.class).setChattingAccount(MsgService
    ...
    public void onDestroy() {
        ...
        NIMClient.getService(MsgServiceObserve.class).observeCustomNotification(commandObserver,
                false);
    ...
    private void parseIntent() {
        ...
        if (!TextUtils.isEmpty(extra)) {
            ...
            dtUserId = jsonExtra.getString("id");
            adminUserId = jsonExtra.getString("adminUserId");
            isAllowSendMsg = jsonExtra.containsKey("isFriend") && jsonExtra.getBoolean("isFriend");
        }
        ...
        customization = (SessionCustomization) getArguments().getSerializable(Extras
                .EXTRA_CUSTOMIZATION);
        ...
        if (customization != null) {
            messageListPanel.setChattingBackground(customization.backgroundUri, customization
                    .backgroundColor);
        }
    }
    ...
    Observer<CustomNotification> commandObserver = new Observer<CustomNotification>() {
        ...
        public void onEvent(CustomNotification message) {
            if (sessionId.equals(message.getSessionId()) && message.getSessionType() !=
                    SessionTypeEnum.P2P) {
                return;
            } else if (message.getFromAccount().equals(adminUserId)) {
                ...
                if (jsonObject.containsKey("type")) {
                    ...
                    if (0 == type) {
                        isAllowSendMsg = false;
                        IMMessage tipMessage = MessageBuilder.createTipMessage(sessionId,
                                SessionTypeEnum.P2P);
                        tipMessage.setFromAccount(sessionId);
                        tipMessage.setContent("对方已把您删除");
    ...
    private Observer<List<MessageReceipt>> messageReceiptObserver = new
    ...
    public boolean sendMessage(IMMessage message) {
        ...
        extendParam.put("id", dtUserId);
        message.setRemoteExtension(extendParam);
        appendPushConfig(message);
        if (!isAllowSendMessage(message)) {
            Toast.makeText(getActivity(), "您与对方不是好友，无法发送消息", Toast.LENGTH_SHORT).show();
